...
// Based on https://www.youtube.com/watch?v=Yq0SfuiOVYE
...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
        ...
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    ...
                    //mutate weight value 
                    float randomNumber = UnityEngine.Random.Range(0f,500f);
        ...
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    if (strWeights == "")
                    {
                        strWeights = weights[i][j][k].ToString();
                    }
                    else
                    {
                        strWeights += ";" + weights[i][j][k].ToString();
    ...
    public void LoadWeights(string strWeights)
    {
        int w = 0;
        string[] strWeight = strWeights.Split(';');
        ...
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    weights[i][j][k] = float.Parse(strWeight[w]);
                    w++;
                }
            }
        }
